...
1. 初始化
   ...
   Webpack 在启动时读取配置文件，初始化必要的参数。

   关键任务：

   - 读取配置文件：从 webpack.config.js 或默认配置中获取打包配置。
   - 初始化 Compiler 对象：
     核心对象 Compiler 被创建，保存所有的配置。
   - 加载插件：
     调用插件的 apply 方法，将插件挂载到生命周期钩子中。
   - 关键钩子：
     - initialize：初始化阶段，加载配置与插件。

2. 编译

   Webpack 从入口文件开始构建依赖图，并依次对模块进行编译。

   关键任务：

   - 入口解析：
     - 从配置中指定的 entry 开始解析依赖关系。
   - 加载 Loader：
     - 根据配置的 module.rules，通过 Loader 对文件进行转换（如 Babel、SWC 等）。
   - 模块解析：
     - 将模块路径（如 import 或 require）解析为物理路径。
...
3. 构建模块依赖图
   ...
   Webpack 递归解析所有模块并生成依赖图。

   关键任务：

   - 模块解析：
     - 递归处理模块的依赖关系（如 import 和 require）。
   - 模块加载与转换：
     - 使用对应的 Loader 转换模块为 JavaScript 格式（如 SCSS 转 CSS）。
   - 依赖图构建：
     - 创建一个模块依赖图，描述各模块的关系及依赖。

   关键钩子：

   - normalModuleFactory：处理模块加载。
   - contextModuleFactory：处理动态模块加载。

4. 生成代码块

   Webpack 将模块分组为代码块（Chunks），用于优化打包效率和输出结构。

   关键任务：

   - 分块处理：
     - 根据配置（如 optimization.splitChunks）将模块拆分为不同的代码块。
   - 优化代码块：
     - 移除冗余代码，合并相似代码。
   - 标识入口和输出点：
     - 确定每个代码块的入口模块及输出文件名。
   ...
   - seal：模块解析完成，进入代码块生成阶段。
   - optimizeChunks：优化代码块。

5. 输出

   Webpack 根据依赖图生成最终的输出文件。
   ...
   - 代码生成：
     - 根据模块依赖图，将模块打包为文件。
   - 模板替换：
     - 使用模板（如 html-webpack-plugin）生成 HTML 文件，插入打包后的文件。
   - 文件输出：
     - 将打包后的文件写入指定目录（如 dist/）。
...
6. 插件处理
   ...
   在整个流程中，Webpack 插件通过生命周期钩子插入自定义逻辑。

   关键任务：

   - 钩子注入：
     - 插件在各个阶段挂载自定义逻辑（如压缩、分析、替换等）。
   - 扩展功能：
     - 如 uglifyjs-webpack-plugin 用于代码压缩、mini-css-extract-plugin 用于分离 CSS。
